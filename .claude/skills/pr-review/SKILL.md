...
Classify the new spec before running checks:

```bash
jq '{dataSource: .dataSource, dataSourceOnly: .dataSourceOnly}' duplocloud/specs/<name>.json
```

| Result | Classification |
|---|---|
| `dataSourceOnly: true` | **Data-source-only** — no managed resource is registered. Adjust the Examples and Generated-docs checks below. |
| `dataSource: true` (no `dataSourceOnly`) | **Hybrid** — managed resource + companion data source. All standard checks apply plus the data-source example. |
| Neither | **Resource-only** — standard checks apply; no data-source example required. |
...
- [ ] **`deprovisionedState` required with deprovision.** If `endpoint.deprovision`
      ...
      ```
- [ ] **Examples** — required files depend on spec classification:
  - **Resource-only or Hybrid:** `examples/resources/duploai_<name>/` must contain **both** `resource.tf` and `import.sh`.
  - **Hybrid** (`dataSource: true`) or **Data-source-only** (`dataSourceOnly: true`): `examples/data-sources/duploai_<name>/data-source.tf` must exist.
  - **Data-source-only** specs do **not** require a `examples/resources/` folder — there is no managed resource to provision or import.
  
  Missing any required file causes `go generate` drift and fails the `Generate` CI check.
- [ ] **Generated docs** — required doc files depend on spec classification:
  - **Resource-only or Hybrid:** `docs/resources/<name>.md` must exist.
  - **Hybrid** or **Data-source-only:** `docs/data-sources/<name>.md` must exist.
  - **Data-source-only** specs do **not** require `docs/resources/<name>.md`.
  
  Every generated doc must carry the tfplugindocs banner:
  `# generated by https://github.com/hashicorp/terraform-plugin-docs`.
  Docs must be generated via `make doc`, not hand-authored.
...
- [ ] **Composite id consistent** — `idPath` in the spec and the docs agree. For
      resource-only or hybrid specs, also verify the `workspace_id/id` format in
      `import.sh` matches. Skip `import.sh` for **data-source-only** specs — there
      is no managed resource to import.
