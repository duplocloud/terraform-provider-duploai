...
# Cron job that opts out of the Resource-Group nodeSelector. By default a cron
# job's pods are pinned to nodes labeled for its resource group (key
# "resourcegroup") — if that resource group has no dedicated node group, the
# pods stay Pending forever. Set is_any_host_allowed = true to let them schedule
# on any node instead. allocation_tags further narrows that to nodes whose node
# group was provisioned with a matching allocation tag.
#
# Unlike duploai_k8s_job, which is immutable, a cron job applies both changes in
# place — so retargeting a schedule's placement does not recreate it.
resource "duploai_k8s_cron_job" "any_host" {
  workspace_id      = "<workspace-id>"
  name              = "shared-nightly-cleanup"
  environment_id    = "<environment-id>"
  resource_group_id = "<resource-group-id>"
  namespace_name    = "default"
