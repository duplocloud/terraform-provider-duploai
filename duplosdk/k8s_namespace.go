...
func init() {
	...
	RegisterEndpoint("k8s_namespace", Endpoint{
		UriBase: base,
		Create:  Operation{Verb: http.MethodPost, Path: ""},     // POST   {base}
		Read:    Operation{Verb: http.MethodGet, Path: "/{id}"}, // GET    {base}/{id}
		// No Update: a namespace is immutable. Any change to its inputs forces
		// replacement; the API exposes no meaningful in-place update.
