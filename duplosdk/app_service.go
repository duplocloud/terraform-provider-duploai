...
// App service (Kubernetes Deployment) API endpoint — the single source of truth
// for this resource's URIs and verbs. The Terraform schema lives in the JSON
// spec.
func init() {
	const base = "/v1/aiservicedesk/user/data/workspaces/{workspace_id}/environment/appservices"

	RegisterEndpoint("app_service", Endpoint{
		UriBase: base,
		...
		// The API rejects a delete while the resource is live, so tear it down
