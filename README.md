...
| [`duploai_network_baseline_security_group_rule`](docs/data-sources/network_baseline_security_group_rule.md) | Manages one inbound rule on a duploai_network_baseline's primary AWS security group — the network-scoped sibling of duploai_resource_group_security_group_rule. Each resource is a single rule with exactly one source: an IPv4 CIDR, an IPv6 CIDR, or a resource group whose own security group becomes the source. Declare several blocks for several rules. AWS assigns each rule an id, which is what Terraform tracks and what the API updates and deletes by |
...
| [`duploai_plan_kms_key`](docs/data-sources/plan_kms_key.md) | Registers an existing, customer-managed KMS key against a duploai_plan, making it selectable by every resource group in the environments that plan is attached to. Resources such as duploai_node_group, duploai_s3_bucket and duploai_rds_instance reference it through their own kms_key_id attribute; the backend resolves that value against the resource group's own registry first, then the plan-level registry. Register a key here rather than per resource group when one key should serve a whole environment. Registration does not grant any permissions on the key — its own key policy must already trust the account and role that will use it |
...
| [`duploai_resource_group_kms_key`](docs/data-sources/resource_group_kms_key.md) | Registers an existing, customer-managed KMS key against a duploai_resource_group. Downstream resources (e.g. duploai_ecr, duploai_aws_efs, duploai_elasticache, duploai_node_group) can then reference the key via their own kms_key_id attribute; the backend resolves it against this registry. Registering a key does not grant the resource group's IAM role any permissions on it — the key's own key policy must already trust the account/role |
| [`duploai_resource_group_security_group_rule`](docs/data-sources/resource_group_security_group_rule.md) | Manages one inbound rule on a duploai_resource_group's primary AWS security group. Each resource is a single rule with exactly one source: an IPv4 CIDR, an IPv6 CIDR, or another resource group whose own security group becomes the source. Declare several blocks for several rules. AWS assigns each rule an id, which is what Terraform tracks and what the API updates and deletes by |
